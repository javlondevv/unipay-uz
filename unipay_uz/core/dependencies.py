"""
Dependency checker and installer for UniPay UZ integrations.
"""
import sys
import warnings
import subprocess
...
def check_dependencies(framework: str, raise_error: bool = False) -> bool:
    ...
    if missing_packages:
        error_msg = (
            f"\n{'='*70}\n"
            f"UniPay UZ: Missing dependencies for {framework.upper()}!\n"
            f"{'='*70}\n"
...
def install_framework_dependencies(framework: str) -> bool:
    ...
    try:
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", f"unipay_uz[{extras_map[framework]}]"],
