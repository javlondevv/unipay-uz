...
from unipay_uz.core.exceptions import (
    PermissionDenied,
    InvalidAmount,
    TransactionNotFound,
    AccountNotFound,
...
from unipay_uz.core.base import BasePaymentProcessor

from unipay_uz.integrations.django.models import PaymentTransaction
from unipay_uz.gateways.paynet.constants import PaynetErrors
...
class PaynetWebhook(BasePaymentProcessor, View):
    ...
    def __init__(self, **kwargs):
        ...
        paynet_settings = settings.TOLOV.get('PAYNET', {})
        ...
        try:
            ...
            logger.error(
                "Could not import %s. Check TOLOV.PAYNET.ACCOUNT_MODEL setting.",
                account_model_path
            )
    ...
    def _perform_transaction(self, params, rpc_id):
        ...
        # Amount check
        # Convert account amount to tiyin (if stored in soum)
        # Verify strict amount match
        if hasattr(account, 'amount'):
            account_amount_tiyin = int(account.amount * 100) # Assuming account.amount is in soum
